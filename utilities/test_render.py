...
def main():
    ...
    # If not found try MATERIALX_DEFAULT_VIEWER environment variable
    if not render_cmd:
        render_cmd = os.getenv('MATERIALX_DEFAULT_VIEWER')
        if not render_cmd:
            print('MaterialXView command not speified or MATERIALX_DEFAULT_VIEWER environment variable not set')
            return
